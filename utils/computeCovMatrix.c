...
/* Function that computes the GP covariance matrix */
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
	...
	/* We initialize the c pointers to the matlab variables */
	...
	ret_matlab = mxCreateDoubleMatrix(nrow, nrow, mxREAL);
	...
	/********** We add some small amount of noise to the diagonal ************************************/

	for (i = 0 ; i < nrow ; i++)
		ret[ i + i * nrow ] = ret[ i + i * nrow ] + sigma * 1e-10;
